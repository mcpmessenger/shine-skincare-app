#!/usr/bin/env python3
"""
Working ML app that starts with basic functionality and gradually loads ML features
"""
import os
import logging
import gc
...
def load_ml_models_async():
    """Load ML models asynchronously without blocking startup"""
    global _ml_loading, _ml_models
    
    if _ml_loading or _ml_models.get('models_loaded'):
        return
    
    _ml_loading = True
    logger.info("Starting async ML model loading...")
    ...
    try:
        ...
        _ml_models['basic_models'] = True
        
        logger.info("Basic ML models loaded successfully")
...
def root():
    return {
        'message': 'Shine ML API - Working Version',
        'status': 'running',
        'memory_usage_mb': get_memory_usage(),
        'ml_status': 'loading' if _ml_loading else 'ready' if _ml_models.get('models_loaded') else 'not_loaded'
    }

@app.route('/api/health')
def health_check():
    """Health check that works immediately"""
    try:
        # Start ML loading if not already started
        if not _ml_loading and not _ml_models.get('models_loaded'):
            load_ml_models_async()
        
        return jsonify({
            'status': 'healthy',
            'timestamp': datetime.utcnow().isoformat(),
            'message': 'Working ML backend is running!',
            'version': 'working-ml-1.0',
            'memory_usage_mb': get_memory_usage(),
...
def analyze_guest_working():
    """Working guest analysis that responds immediately"""
    try:
        logger.info("Working guest analysis requested")
        ...
        analysis = create_working_analysis(ethnicity)
        ...
        response_data = {
            ...
            'data': {
                ...
                'skin_classification': {
                    ...
                    'ml_model_used': 'working_skin_classifier_v1'
                },
                'enhanced_features': {
                ...
                'message': 'Working ML analysis completed! Sign up to save your results!'
            }
        }
        ...
        logger.info("Working analysis completed successfully")
        return jsonify(response_data)
        
    except Exception as e:
        logger.error(f"Error in working guest analysis: {str(e)}")
        return jsonify({
            'success': False,
...
def classify_skin_type_working():
    """Working skin type classification"""
    try:
        logger.info("Working skin classification requested")
        ...
        classification = create_working_classification(ethnicity)
        ...
        response_data = {
            ...
            'classifier_info': {
                'version': '1.0.0',
                'model': 'working_fitzpatrick_monk',
                'features': ['ethnicity_aware', 'confidence_scoring', 'multi_scale'],
                'memory_usage_mb': get_memory_usage()
            }
        }
        
        logger.info("Working classification completed successfully")
        return jsonify(response_data)
        
    except Exception as e:
        logger.error(f"Error in working classification: {str(e)}")
        return jsonify({
            'success': False,
            'error': 'Internal server error'
        }), 500
...
def enhanced_health_working():
    """Working enhanced health check"""
    try:
        return jsonify({
            'status': 'healthy',
            'timestamp': datetime.utcnow().isoformat(),
            'message': 'Working enhanced backend is running!',
            'memory_usage_mb': get_memory_usage(),
            'ml_models_loaded': _ml_models.get('models_loaded', False),
            'ml_loading': _ml_loading,
            'services': {
...
def create_working_analysis(ethnicity=''):
    """Create working analysis with enhanced features"""
    ...
    return {
        ...
        'ml_features': {
            'model_version': 'working_v1.0',
            'processing_time_ms': 800,
            'memory_optimized': True
        }
    }

def create_working_classification(ethnicity=''):
    """Create working skin classification"""
    ...
    return {
        ...
        'confidence': 0.85,
        'ethnicity_considered': bool(ethnicity),
        'classification_method': 'working_ml_ai',
        'features_analyzed': [
