...
Write-Host "`n🔍 Checking current deployment status..." -ForegroundColor Cyan
...
try {
    ...
    if ($envInfo.Environments.Count -gt 0) {
        ...
        if ($env.Status -eq "Updating") {
            Write-Host "⚠️ Environment is currently updating (hanging). Terminating..." -ForegroundColor Yellow
            
            # Terminate the environment
            Write-Host "🗑️ Terminating environment..." -ForegroundColor Red
...
Write-Host "`n🧹 Cleaning up orphaned resources..." -ForegroundColor Cyan
...
Write-Host "`n⏳ Waiting 60 seconds before allowing new deployment..." -ForegroundColor Cyan
